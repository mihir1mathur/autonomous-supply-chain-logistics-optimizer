...
Source: simulation/delivery_routes.csv (Stage 2).
...
it goes to. The vehicle_id column is reserved for LATER (OR-Tools, Stage 3+):
the simple Stage 2 estimate does not assign a vehicle, so it stays NULL for now.
"""
...
class DeliveryRoute(Base):
    ...
    # Stage 2 estimate has no vehicle assignment; the optimizer fills it later.
    ...
    # Estimates (COMPUTED in Stage 2; the optimizer improves these later).
    ...
    # Disruptions that specifically delay THIS route (reserved for Stage 7;
    # empty for now because Stage 2 disruptions are area/warehouse level).
