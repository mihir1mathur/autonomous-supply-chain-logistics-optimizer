"""
============================================================================
STAGE 6 - BENCHMARK RUNNER  (Part 7)
...
---------------------
  ...
  The default sweep is the Stage 6 benchmark set (optimization/scenarios.py,
  BENCHMARK_SCENARIOS):
  ...
  API, collects the KPIs, and then writes a single report to:
        benchmarks/benchmark_report.md    (human-readable table)
        benchmarks/benchmark_report.json   (machine-readable, full detail)

REPRODUCIBILITY (Stage 6 reproducibility fix)
--------------------------------------------
...
-------------
        ...
        python notebooks/benchmark_runner.py
...
MD_PATH = os.path.join(BENCHMARK_DIR, "benchmark_report.md")
JSON_PATH = os.path.join(BENCHMARK_DIR, "benchmark_report.json")
...
def build_markdown(runs, meta) -> str:
    ...
    lines = []
    lines.append("# Stage 6 Benchmark Report")
    lines.append("")
    lines.append(
    ...
    lines.append("")
    lines.append(
        "> Generated by `notebooks/benchmark_runner.py`. Every run above is "
...
def main():
    banner("STAGE 6 - BENCHMARK RUNNER")
    client = get_client()
