"""
============================================================================
EXECUTION CONFIG  (Stage 6)   -- tunable settings for the EXECUTION layer
Project: Supply Chain & Logistics Optimizer
============================================================================

WHAT THIS FILE DOES
-------------------
  Stage 5 gave the project an optimization ENGINE (optimization/config.py holds
  its solver settings). Stage 6 wraps that engine in an EXECUTION LAYER that
  ...
  Those numbers live HERE, in one typed settings object, exactly like the Stage
  4 API config (api/config.py) and the Stage 5 optimization config
  (optimization/config.py). Keeping Stage 6's tunables in their OWN file means
  the Stage 5 config is never touched - this file is purely additive.

WHY A SECOND SETTINGS OBJECT (and not editing optimization/config.py)
---------------------------------------------------------------------
  The project's rule is that each stage is ADDITIVE and never rewrites earlier
  work. optimization/config.py belongs to Stage 5 and stays exactly as it was.
  This file adds the Stage 6 knobs alongside it. Both read from the same .env
  with the shared OPT_ prefix; `extra="ignore"` lets each object ignore the
  other's variables, so they coexist cleanly.

HONESTY ABOUT SIMULATED COSTS (same discipline as Stage 2)
---------------------------------------------------------
  ...
  discipline. The per-km default (1.20) matches the rate Stage 2 used to compute
...
class ExecutionSettings(BaseSettings):
    """
    Stage 6 execution-layer settings (KPI pricing + reporting thresholds).
    ...
    # vehicle). 1.20 matches the rate Stage 2 used for estimated_cost.
    ...
    # same 0.90 Stage 5 uses for "overloaded" so the two ideas agree.
...
def get_execution_settings() -> ExecutionSettings:
    ...
    get_settings() (Stage 4) and get_optimization_settings() (Stage 5).
    """
