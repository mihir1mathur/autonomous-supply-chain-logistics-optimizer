...
-----------------------------------------------------------
  ...
  database code from Stage 3. It re-exports the Stage 3 engine / session factory
  / Base, and adds ONE new thing FastAPI needs: a per-request session
  "dependency" called get_db().

  It deliberately duplicates NONE of Stage 3's logic. The engine, the
  ...
  change it in ONE place (Stage 3) and the API follows automatically.

WHY DOES FASTAPI NEED A SPECIAL SESSION FUNCTION?
-------------------------------------------------
  A database SESSION is one short conversation with the database (Stage 3
...
# REUSE Stage 3 - do not re-create any of this. Importing database.connection
...
def get_db() -> Generator[Session, None, None]:
    ...
    This mirrors the Stage 3 get_session() context manager, but in the shape
