# API Architecture (Stage 4)

This document explains the **backend API layer** introduced in Stage 4: why it
...
- [`database_design.md`](database_design.md) — the Stage 3 database this API sits
...
Stage 3 put the data into **PostgreSQL** behind a clean **SQLAlchemy** layer.
...
and receive JSON describing that warehouse. Stage 4 builds that API on top of the
Stage 3 database **without duplicating any database logic** — it reuses the same
...
- **Small, readable code.** Endpoints are plain Python functions with type
  hints, which keeps the codebase approachable and easy to review.
...
```
   ...
   |                   |   - reuses the Stage 3 CRUD helpers
   +-------------------+
          |
          v
   +-------------------+
   |   SQLAlchemy      |   models/*.py + database/ (Stage 3, REUSED)
   |   (ORM)           |   - Python objects mapped to tables
   +-------------------+
          |
          v
   +-------------------+
   |   PostgreSQL      |   the Stage 3 database (unchanged)
   +-------------------+
          |
          v
...
stages change one layer without disturbing the others — for example, adding a
...
├── database.py       # REUSES the Stage 3 engine/session; get_db() per request
...
Seven entities are exposed, matching the Stage 3 tables: **customers,
...
7. **SQLAlchemy + PostgreSQL.** The query runs against the Stage 3 database.
...
|-------------------|-------------------|--------------------------|
| **OR-Tools** route optimization (Stage 5) | a new service that writes optimized `vehicle_id`/estimates into the reserved route columns | routers and schemas already expose routes; the DB columns are already reserved (Stage 3) |
| **Redis** caching (Stage 6) | wrap hot reads inside services / a cache dependency | routers and the DB are untouched |
| **CrewAI** agents (Stage 7) | agents call the same service functions | services are the shared, reusable logic |
| **Authentication** | a `current_user` router dependency + 401/403 (already reserved) | endpoints only *declare* the dependency |
| **Dashboard** (Stage 8) | reads the existing JSON endpoints | the API is already language-neutral |
| **AWS** deployment (Stage 9) | point the DB URL at a managed PostgreSQL via `.env` | the engine URL is the only change |
...
implementation**, **the Stage 3 database layer reused unchanged**, **dependency
...
Stage 4 is one step in an incremental build. Each stage builds directly on the
outputs of the previous one:

```
Stage 0   Understand the dataset          (real Olist e-commerce data)
   |
Stage 1   Clean + join the data           (processed/ master table)
   |
Stage 2   Logistics simulation            (warehouses, inventory, vehicles,
   |                                       routes, disruptions)
Stage 3   PostgreSQL database             (SQLAlchemy models + CRUD)
   |
Stage 4   FastAPI backend  <-- THIS WEEK  (REST API reusing Stage 3)
   |
Stage 5   Route optimization (OR-Tools)   (writes back into reserved columns)
   |
Stage 6   Redis caching                   (in front of hot API reads)
   |
Stage 7   CrewAI multi-agent system       (agents call the services)
   |
Stage 8   Monitoring & evaluation metrics (measure the system)
   |
Stage 9   AWS deployment                  (managed PostgreSQL + hosting)
   |
Stage 10  Production system               (the full platform)
```

Stage 4 turns the stored data of Stage 3 into a shared, documented, network-
reachable service — the surface every later stage reads from and writes to.
