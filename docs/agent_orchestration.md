# AI Multi-Agent Orchestration Layer (Stage 7)
Project: Supply Chain & Logistics Optimizer

Stage 6 gave the project a complete optimization **execution layer**: one service
...
stores the run. Stage 7 puts an **AI multi-agent orchestration layer** on **top**
of that: five specialised agents that read a plain-language request, decide
**what** should happen, drive the Stage 6 service to make it happen, judge the
...
> **Everything in Stage 7 is additive.** No Stage 0–6 file is rewritten. The
> agents live in a new `agents/` package and a new `/agents/*` API namespace,
> and they **reuse the Stage 6 execution service unchanged** — the agents never
...
Every stage of this project follows one small story: **a customer orders a phone
charger**. Stages 0–2 modelled the charger's journey (order → warehouse → packed
→ route → delivered). Stage 3 stored it, Stage 4 served it over HTTP, Stage 5 built
the optimizers that decide *which van carries the charger and by what route*,
and Stage 6 wrapped those optimizers so we could run a scenario, score it, and
save it. Stage 7 asks the natural next question: instead of a human choosing the
...
The Stage 6 service is powerful but *low-level*: to use it you must already know
...
coordination layer** that turns intent into the right Stage 6 call and explains
the outcome.
...
| **Scenario** | `agents/scenario_agent.py` | Choose **one existing Stage 6 scenario** to run under (holiday, vehicle_breakdown, …). Never invents a scenario. | `ScenarioDecision` |
| **Optimization** | `agents/optimization_agent.py` | **Drive the Stage 6 execution service** through the tool. The "hands" of the crew — thin on purpose. | `OptimizationOutcome` |
...
**template-method** idea as the Stage 4 `BaseService`: the shell lives in one
...
The layering rule from Stage 4 is preserved end to end — each layer only talks to
...
```
        User  (browser, dashboard, script, curl)
          ...
          ▼
   api/routers/agents.py            THIN router (HTTP only)          [Stage 7, new]
          ▼
   api/services/agent_service.py    the agent bridge service          [Stage 7, new]
          ▼
   agents/coordinator.py            the ORCHESTRATOR (the "crew")      [Stage 7, new]
          ▼
   Planner → Scenario → Optimization → Evaluation → Reporting         [Stage 7, new]
          ▼
   agents/tools.py                  the ONLY seam to the platform      [Stage 7, new]
          ▼
   api/services/execution_service.py   the Stage 6 EXECUTION LAYER      [Stage 6, reused]
          ▼
   optimization/*_solver.py            the Stage 5 OR-Tools ENGINE      [Stage 5, reused]
          ▼
   optimization_runs table  +  the Stage 3 database (PostgreSQL)        [Stage 3/6]
```
...
the trusted, tested Stage 6 service — never re-implemented and never bypassed. The
...
Every earlier stage runs **offline and deterministically**: the API starts with
...
network call, and it is not deterministic. So Stage 7 supports **two modes** and
...
- **`deterministic`** — the **default**, always-available mode. The five agents
  ...
  always come from the Stage 6 execution service.
...
execution layer). Stage 7 adds a **distinct `/agents/*` namespace** for the AI
layer, so the earlier stages are untouched and the concerns never collide. The
...
default — the same "empty body works" convention as Stages 5 and 6.
...
> the body is still a clean `422` via the Stage 4 validation handler.)
...
exactly like the Stage 5 solution models and the Stage 6 `RunMetrics` /
...
| `ScenarioDecision` | Scenario | `key`, `name`, `category`, `description`, `rationale` (the key is validated against the live Stage 6 catalog) |
...
planned Stage 8 dashboard — can follow after the fact.

---

## What is reused, unchanged

| From | Reused by Stage 7 |
|------|------------------|
| Stage 6 | the entire `execution_service` (`run` / `simulate` / `run_benchmark` / `list_scenarios`), the scenario catalog, the 12 KPIs, and the before-vs-after evaluation |
| Stage 5 | the four OR-Tools solvers — reached only indirectly, through Stage 6 |
| Stage 4 | the thin-router rule, dependency-injected `get_db`, and the JSON error envelope |
| Stage 3 | the SQLAlchemy models and the lazy engine/`SessionLocal` (the tool opens/closes its own session only when the API does not pass one) |
...
```bash
python database/init_db.py                    # ensure optimization_runs exists (Stage 6)
