# Optimization Execution Layer (Stage 6)

Stage 5 gave the project an optimization **engine** (`optimization/`) and a bridge
service that could **run a solver on demand** behind `/optimize/*`. Stage 6 wraps
...
This document describes the Stage 6 architecture and runtime flow. It complements
the Stage 5 [`optimization_architecture.md`](optimization_architecture.md) (the
engine's internal design), and the three companion Stage 6 docs:
...
> **Everything in Stage 6 is additive.** No Stage 0–5 file is rewritten. The Stage 5
> `/optimize/*` endpoints and the `optimization_service` are untouched; Stage 6
...
Client  (browser, dashboard, agent, script)
  ...
  ▼
api/routers/execution.py         THIN router (HTTP only)         [Stage 6, new]
  ▼
api/services/execution_service.py    the EXECUTION LAYER          [Stage 6, new]
  │   1. load inputs from the DB (Stage 3 models)
  │   2. apply a SCENARIO            optimization/scenarios.py     [Stage 6, new]
  │   3. SOLVE                       optimization/*_solver.py      [Stage 5, reused]
  │   4. MEASURE KPIs                optimization/metrics.py       [Stage 6, new]
  │   5. EVALUATE before vs after    optimization/evaluation.py    [Stage 6, new]
  │   6. STORE the run               models/optimization_run.py    [Stage 6, new]
  ▼
optimization_runs table  ◄── read back by ──►  api/services/optimization_run_service.py
  (PostgreSQL, Stage 3 engine)                   (subclasses the Stage 4 BaseService)
```

The layering rule from Stage 4 is preserved: **routers are thin**, all business
logic lives in the service layer, and only the service touches the database and
the engine. The Stage 5 engine stays **database-free and FastAPI-free** — the
...
All under the `/optimization` prefix (distinct from Stage 5's `/optimize`).
...
> The Stage 6 goals list `POST /optimize` and `POST /simulate`. Because Stage 5
> already owns the `/optimize` prefix, Stage 6 groups its endpoints under
> `/optimization` to avoid clobbering Stage 5 — so `POST /optimize` maps to
...
Router: run_optimization(payload)
  -> ExecutionService.run(db, optimizer, scenario, ...)
       ...
       2. LOAD    the real inputs from the Stage 3 DB (warehouses, vehicles,
                  inventory, delivery routes) - the same rows Stage 5 reads
       3. SCENARIO apply_scenario(scenario, shipments, vehicles, warehouses, stock)
                  -> modified COPIES + a list of what changed
       4. SOLVE   the reused Stage 5 solver (assignment/fleet/routes/warehouse)
...
after Stage 6 adds `optimization_runs` and leaves every Stage 3 table untouched
(**no migration**).
...
The read side reuses the Stage 4 `BaseService` unchanged:
...
| From | Reused by Stage 6 |
|------|------------------|
| Stage 5 | the four solver singletons (`assignment_solver`, `vehicle_optimizer`, `route_optimizer`, `warehouse_selector`) and the input/output dataclasses |
| Stage 5 | the `OPT_*` caps (`max_shipments_per_request`, …) and the winding factor |
| Stage 4 | `BaseService` (list/get/paginate), the pagination envelope, and the one-JSON-error envelope + handlers (a bad optimizer → 400, missing warehouse → 404, invalid body → 422) |
| Stage 3 | the SQLAlchemy models and the lazy engine/session |
| Stage 2 | the haversine × 1.30 winding-factor distance model and the `estimated_distance_km` leg distances |

---

## Errors

Optimization-execution errors flow through the **same** Stage 4 exception
...
python notebooks/execution_demo.py      # run / simulate / history / metrics
python notebooks/benchmark_runner.py    # sweep the benchmark scenarios
python notebooks/execution_validation.py          # the Stage 6 PASS/FAIL checklist
```
